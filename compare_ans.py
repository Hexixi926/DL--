...
def extract_boxed_answer(text: str, box_string :str="\\boxed"):
    def last_boxed_only_string(text):
        idx = text.rfind(box_string)
        if idx < 0:
            idx = text.rfind("\fbox")
            if idx < 0:
                return None
...
def extract_answer(text: str) -> str:
    ...
    Args:
        text: The input text containing the answer.
    '''
    if extract_boxed_answer(text, "\\boxed"):
        return extract_boxed_answer(text, "\\boxed")
    if extract_boxed_answer(text, "\boxed"):
        return extract_boxed_answer(text, "\boxed")
    return ""

def judge_answer_list(preds: list, gts: list) -> int:
    '''
    Judge whether the answer is correct.
    Args:
        preds: The predicted answers.
        gts: The ground truth answers.
    '''
    correct = 0
    for pred in preds:
        if any(
            math_equal(pred, gt, timeout=False) for gt in gts
        ):
            correct += 1
    return correct


def judge_answer(pred: str, gt: str) -> bool:
    '''
    Judge whether the answer is correct.
    Args:
    ...
    '''
    return math_equal(extract_answer(pred), gt, timeout=True)
...
def _test_math_equal():
    ...
    gt = r"\[ \boxed{\textbf{(A)} 5 \qquad \textbf{(B)} 6 \qquad \textbf{(C)} 120\qquad \textbf{(D)} 2^{10} \qquad \textbf{(E)} \text{none of these} \]"
    pred = r"a \in [2, +\infty)"

    # 1. Extract the boxed answer
    # 2. Compare the extracted answer with the ground truth
    print(math_equal(extract_answer(pred), gt, timeout=True))
